...
<?php foreach ($this->configuration->getValue('list.display') as $name => $field): ?>
  ...
  [?php if ('<?php echo $name ?>' == $sort[0]): ?]
    ...
    [?php else: ?]
    [?php echo link_to(
      image_tag(((sfConfig::get('app_aAdmin_web_dir',false))?sfConfig::get('app_aAdmin_web_dir'):'/apostrophePlugin').'/images/'.$sort[1].'.png', array('alt' => __($sort[1], array(), 'a-admin'), 'title' => __($sort[1], array(), 'a-admin'))),
      '<?php echo $this->getModuleName() ?>/index?sort=<?php echo $name ?>&sort_type='.($sort[1] == 'asc' ? 'desc' : 'asc')
      ) ?]
  [?php endif; ?]
