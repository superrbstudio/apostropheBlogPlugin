...
<?php $filterUrl = aUrl::addParams('aEvent/index', array('tag' => $sf_params->get('tag'), 'cat' => $sf_params->get('cat'), 'year' => $sf_params->get('year'), 'month' => $sf_params->get('month'), 'day' => $sf_params->get('day'), 'q' => $sf_params->get('q'))) ?>
...
<?php if((count($categories) > 1) || ($sf_params->get('cat'))): ?>
<hr class="a-hr" />
  ...
  <div class="a-filter-options blog clearfix">
	  <?php foreach ($categories as $category): ?>
	    <div class="a-filter-option">
				<?php $selected_category = ($category->getSlug() == $sf_params->get('cat')) ? $selected : array() ?>
				<?php echo a_button($category, ($sf_params->get('cat') == $category->getSlug()) ? url_for(aUrl::addParams($filterUrl, array('cat' => ''))) : url_for(aUrl::addParams($filterUrl, array('cat' => $category->slug))), array_merge(array('a-link'),$selected_category)) ?>
			</div>
	  <?php endforeach ?>
  </div>	
</div>
...
<div class="a-subnav-section tags">  
	...
	<ul class="a-ui a-tag-sidebar-list popular">
	  ...
	  <li <?php echo ($n == count($popular) ? 'class="last"':'') ?>>
			<?php echo a_button('<span class="a-tag-count">'.$count.'</span>'.$tag, url_for(aUrl::addParams($filterUrl, array('tag' => $tag))), array('a-link','a-tag')) ?>
		</li>
		<?php $n++; endforeach ?>
	</ul>
...
<?php if(!isset($noFeed)): ?>
  ...
  <?php $filtered = url_for(aUrl::addParams($filterUrl, array('feed' => 'rss', 'year' => '', 'month' => '', 'day' => ''))) ?>
