<div class="a-blog-categories">
  ...
  <?php foreach ($categories as $category): ?>
    <li><?php echo link_to($category, aUrl::addParams(aTools::getCurrentPage()->getUrl().'?cat='.$category->getSlug(), $params['cat']), array(
...
<div class="a-blog-filter">
  <h4>Browse by</h4>
  <ul class="a-blog-filter-options">
    <li><?php echo link_to('Day', aTools::getCurrentPage()->getUrl().'?'.http_build_query(($dateRange == 'day') ? $params['nodate'] : $params['day']), array('class' => ($dateRange == 'day') ? 'selected' : '')) ?></li>
    <li><?php echo link_to('Month', aTools::getCurrentPage()->getUrl().'?'.http_build_query(($dateRange == 'month') ? $params['nodate'] : $params['month']), array('class' => ($dateRange == 'month') ? 'selected' : '')) ?></li>
    <li><?php echo link_to('Year', aTools::getCurrentPage()->getUrl().'?'.http_build_query(($dateRange == 'year') ? $params['nodate'] : $params['year']), array('class' => ($dateRange == 'year') ? 'selected' : '')) ?></li>
  </ul>
</div>
