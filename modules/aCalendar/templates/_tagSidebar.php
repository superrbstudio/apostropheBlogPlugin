...
<div class="a-subnav-section categories">
  <h4>Categories</h4>
	<?php if (count($categories)): ?>
	  <ul class="a-filter-options events">
	  <?php foreach ($categories as $category): ?>
	    <li class="a-filter-option"><?php echo link_to($category, aUrl::addParams(($sf_params->get('cat') == $category->getSlug()) ? 'aCalendar/index' : 'aCalendar/index?cat='.$category->getSlug(), $params['cat']), array(
...
<div class="a-subnav-section range">
  ...
  <ul class="a-filter-options blog">
    <li class="a-filter-option"><?php echo link_to('Day', 'aCalendar/index?'.http_build_query(($dateRange == 'day') ? $params['nodate'] : $params['day']), array('class' => ($dateRange == 'day') ? 'selected' : '')) ?></li>
    <li class="a-filter-option"><?php echo link_to('Month', 'aCalendar/index?'.http_build_query(($dateRange == 'month') ? $params['nodate'] : $params['month']), array('class' => ($dateRange == 'month') ? 'selected' : '')) ?></li>
    <li class="a-filter-option"><?php echo link_to('Year', 'aCalendar/index?'.http_build_query(($dateRange == 'year') ? $params['nodate'] : $params['year']), array('class' => ($dateRange == 'year') ? 'selected' : '')) ?></li>
  </ul>
</div>
