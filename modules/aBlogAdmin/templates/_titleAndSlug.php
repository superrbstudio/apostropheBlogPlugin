...
<?php use_helper('a') ?>
<form method="POST" action="<?php echo url_for('a_blog_admin_updateTitle', $a_blog_post) ?>" id="a-blog-item-title-interface">
	<input type="text" name="title" class="a-title" value="<?php echo a_entities(($a_blog_post->title == 'untitled')? '':$a_blog_post->title) ?>" />
...
<form method="POST" action="<?php echo url_for('a_blog_admin_updateSlug', $a_blog_post) ?>" id="a-blog-item-permalink-interface">
	<h6>Permalink:</h6>
	...
	<div class="a-blog-item-permalink-wrapper slug">
		<input type="text" name="slug" class="a-slug" value="<?php echo a_entities($a_blog_post->slug) ?>">
